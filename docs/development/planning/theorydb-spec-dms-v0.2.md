# TableTheory Spec (DMS) v0.2 (Draft)
...
DMS v0.2 makes update-field presence explicit. When a high-level update
explicitly selects an `omit_empty: true` attribute and supplies an empty value,
the runtime MUST remove that DynamoDB attribute. DMS v0.1 did not distinguish
an unselected field from a selected empty field, and the runtimes diverged:
TypeScript removed the attribute while Go and Python stored the empty value.

This change does not make old items unreadable and does not require a bulk data
rewrite. It does change the observable result of future explicitly selected
empty updates, including `attribute_exists` conditions, sparse-index
membership, projections, and raw item shape. Consumers migrating from v0.1
must audit any update that explicitly selects an `omit_empty` attribute with an
empty value.
...
DMS is a versioned, language-neutral document. v0.2 is intentionally minimal and focused on high-drift behavior.

### Canonical format (v0.2)
...
### Model write policy fields (v0.2)
...
- `pascalCase` is not a DMS v0.2 naming convention. Runtimes MAY retain language-specific legacy model support, but DMS emitters/validators MUST NOT produce or accept `naming.convention: "pascalCase"`.
...
If `omit_empty: true`, an attribute MUST be omitted from whole-item writes when
its value is empty by these rules:

- `null`/`undefined` (or language-equivalent) is empty.
- strings: `""` is empty.
- numbers: `0` is empty.
- booleans: `false` is empty.
- arrays/lists: length `0` is empty. Fixed-length runtime arrays are non-empty
  whenever their declared length is greater than zero, regardless of element
  values.
- DMS `M` maps/objects/structs/records: size `0` is empty. A non-empty
...
### Explicit update selection and `omit_empty`

High-level partial-update APIs MUST distinguish whether a field was selected
for mutation:

- An unselected field produces no update expression and leaves the persisted
  attribute unchanged.
- A selected `omit_empty: true` field with an empty value MUST produce a
  DynamoDB `REMOVE` action for that attribute.
- A selected field without `omit_empty` MUST store its supplied value,
  including valid DynamoDB empty scalar/list/map values.
- Low-level expression builders MAY expose an explicit `SET` escape hatch, but
...
Envelope shape (v0.2 aligns with `docs/development/planning/theorydb-encryption-tag-roadmap.md`):
...
- v0.2 is a breaking successor to v0.1 because explicitly selected empty
  `omit_empty` updates now have one portable `REMOVE` result.
