...
| Make `tabletheory_py` the canonical Python package | Enumerated item 19 added the `tabletheory_py` import alias; `py/README.md`, `py/docs/getting-started.md`, and `docs/runtimes/python/getting-started.md` identify `tabletheory_py` as canonical while the legacy `theorydb_py` package remains only as a transition warning shim. | THE-2455 / TTIP-117 |
...
`tabletheory_py` is the canonical Python import package. The legacy `theorydb_py` package is no longer the implementation package; this v2 readiness branch keeps it only as a `DeprecationWarning` shim for transition diagnostics.
